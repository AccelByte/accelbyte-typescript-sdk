...
export class OverrideRoleConfigV3Admin$ {
  ...
  getRoleoverride_v3(queryParams: { identity: 'GAME_ADMIN' | 'USER' | 'VIEW_ONLY' }): Promise<Response<RoleOverrideResponse>> {
    ...
    const url = '/iam/iam/v3/admin/namespaces/{namespace}/roleoverride'.replace('{namespace}', this.namespace)
  ...
  getRoleoverrideSource_v3(queryParams: { identity: 'GAME_ADMIN' | 'USER' | 'VIEW_ONLY' }): Promise<Response<RoleOverrideSourceResponse>> {
    const params = { ...queryParams } as AxiosRequestConfig
    const url = '/iam/iam/v3/admin/namespaces/{namespace}/roleoverride/source'.replace('{namespace}', this.namespace)
    const resultPromise = this.axiosInstance.get(url, { params })
  ...
  patchRoleoverrideStatus_v3(
    ...
    const params = { ...queryParams } as AxiosRequestConfig
    const url = '/iam/iam/v3/admin/namespaces/{namespace}/roleoverride/status'.replace('{namespace}', this.namespace)
    const resultPromise = this.axiosInstance.patch(url, data, { params })
  ...
  getPermissions_ByRoleId_v3(roleId: string): Promise<Response<RolePermissionResponseV3>> {
    ...
    const url = '/iam/iam/v3/admin/namespaces/{namespace}/roleoverride/{roleId}/permissions'
