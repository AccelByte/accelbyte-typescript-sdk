...
export class MatchTickets$ {
  ...
  createMatchTicket(data: MatchTicketRequest): Promise<IResponse<MatchTicketResponse>> {
    ...
    return Validate.validateOrReturnResponse(this.isZodEnabled, () => resultPromise, MatchTicketResponse, 'MatchTicketResponse')
  }

  /**
   * Get my match tickets.
   */
  getMatchTicketsMe(queryParams?: { limit?: number; matchPool?: string | null; offset?: number }): Promise<IResponse<MatchTicketStatuses>> {
    const params = { ...queryParams } as SDKRequestConfig
    const url = '/match2/v1/namespaces/{namespace}/match-tickets/me'.replace('{namespace}', this.namespace)
    const resultPromise = this.axiosInstance.get(url, { params })

    return Validate.validateOrReturnResponse(this.isZodEnabled, () => resultPromise, MatchTicketStatuses, 'MatchTicketStatuses')
  }
   ...
   */
  getMatchTicket_ByTicketid(ticketid: string): Promise<IResponse<MatchTicketStatus>> {
    const params = {} as SDKRequestConfig
    const url = '/match2/v1/namespaces/{namespace}/match-tickets/{ticketid}'
      .replace('{namespace}', this.namespace)
      .replace('{ticketid}', ticketid)
    ...
    return Validate.validateOrReturnResponse(this.isZodEnabled, () => resultPromise, MatchTicketStatus, 'MatchTicketStatus')
  }
}
