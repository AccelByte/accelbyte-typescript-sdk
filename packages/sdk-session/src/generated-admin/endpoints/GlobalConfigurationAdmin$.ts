...
export class GlobalConfigurationAdmin$ {
   ...
   */
  getGlobalConfigurations(): Promise<IResponseWithSync<GlobalConfigurationResponse>> {
    const params = {} as SDKRequestConfig
    const url = '/session/v1/admin/global-configurations'
    const resultPromise = this.axiosInstance.get(url, { params })

    const res = () => Validate.responseType(() => resultPromise, GlobalConfigurationResponse, 'GlobalConfigurationResponse')
  ...
  updateGlobalConfiguration(data: PutGlobalConfigurationRequest): Promise<IResponse<GlobalConfigurationResponse>> {
    ...
    return Validate.responseType(() => resultPromise, GlobalConfigurationResponse, 'GlobalConfigurationResponse')
  }
}
