...
export class AdminGroupAdmin$ {
   ...
   */
  getGroups(queryParams?: { limit?: number; offset?: number }): Promise<IResponse<PaginatedGroupResponse>> {
   ...
   */
  getGroup_ByGroupId(groupId: string): Promise<IResponse<CreateGroupResponse>> {
    const params = {} as SDKRequestConfig
    const url = '/ugc/v1/admin/namespaces/{namespace}/groups/{groupId}'.replace('{namespace}', this.namespace).replace('{groupId}', groupId)
    const resultPromise = this.axiosInstance.get(url, { params })
