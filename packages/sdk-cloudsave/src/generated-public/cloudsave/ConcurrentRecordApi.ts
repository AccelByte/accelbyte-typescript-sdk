...
export function ConcurrentRecordApi(sdk: AccelbyteSDK, args?: ApiArgs) {
   ...
   */
  async function updateConcurrentRecord_ByUserId_ByKey(key: string, userId: string, data: ConcurrentRecordRequest): Promise<unknown> {
    const $ = new ConcurrentRecord$(Network.create(requestConfig), namespace, cache)
    const resp = await $.updateConcurrentRecord_ByUserId_ByKey(key, userId, data)
    if (resp.error) throw resp.error
    return resp.response.data
  }
   ...
   */
  async function updatePublicConcurrent_ByUserId_ByKey(key: string, userId: string, data: ConcurrentRecordRequest): Promise<unknown> {
    const $ = new ConcurrentRecord$(Network.create(requestConfig), namespace, cache)
    const resp = await $.updatePublicConcurrent_ByUserId_ByKey(key, userId, data)
    if (resp.error) throw resp.error
    return resp.response.data
  }
