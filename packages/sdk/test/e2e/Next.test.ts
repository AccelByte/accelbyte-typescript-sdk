...
test.describe('Next App', () => {
  ...
  test(`Check if SDK is working - listDiscoveryConfigs`, async () => {
    ...
    test.expect(Configs.safeParse(JSON.parse(content)).success).toBe(true)
  })
  ...
  test(`Check if SDK is working - listOfCurrencies`, async () => {
    ...
    const content = await responseItem.innerHTML()
    test.expect(CurrencyInfo.safeParse(JSON.parse(content)[0]).success).toBe(true)
  })
})
