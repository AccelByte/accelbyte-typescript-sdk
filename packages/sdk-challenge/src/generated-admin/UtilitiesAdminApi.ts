...
export function UtilitiesAdminApi(sdk: AccelByteSDK, args?: SdkSetConfigParam) {
  ...
  async function getChallengesItemReferences(queryParams: { itemId: string | null }): Promise<AxiosResponse<ItemReferenceResponse>> {
    const $ = new UtilitiesAdmin$(axiosInstance, namespace, useSchemaValidation)
    const resp = await $.getChallengesItemReferences(queryParams)
