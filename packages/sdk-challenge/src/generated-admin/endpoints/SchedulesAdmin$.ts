...
export class SchedulesAdmin$ {
  ...
  getSchedules_ByChallengeCode(
    ...
    const url = '/challenge/v1/admin/namespaces/{namespace}/challenges/{challengeCode}/schedules'
  ...
  getSchedules_ByChallengeCode_ByCode(
    ...
    const params = { limit: 20, ...queryParams } as AxiosRequestConfig
    const url = '/challenge/v1/admin/namespaces/{namespace}/challenges/{challengeCode}/goals/{code}/schedules'
      .replace('{namespace}', this.namespace)
      .replace('{challengeCode}', challengeCode)
      .replace('{code}', code)
    const resultPromise = this.axiosInstance.get(url, { params })
