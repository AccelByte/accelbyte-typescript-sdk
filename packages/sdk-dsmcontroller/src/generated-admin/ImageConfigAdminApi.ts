...
export function ImageConfigAdminApi(sdk: AccelbyteSDK, args?: ApiArgs) {
  ...
  async function createImage(data: CreateImageRequest): Promise<unknown> {
    const $ = new ImageConfigAdmin$(Network.create(requestConfig), namespace, cache)
   ...
   */
  async function createImageImport(data: { file: File }): Promise<ImportResponse> {
    const $ = new ImageConfigAdmin$(Network.create(requestConfig), namespace, cache)
    const resp = await $.createImageImport(data)
    if (resp.error) throw resp.error
    return resp.response.data
  }
   ...
   */
  async function getImagesExport(): Promise<ImageRecordArray> {
    const $ = new ImageConfigAdmin$(Network.create(requestConfig), namespace, cache)
    const resp = await $.getImagesExport()
    if (resp.error) throw resp.error
    return resp.response.data
  }
